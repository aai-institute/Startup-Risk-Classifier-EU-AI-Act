...
def multiple_model_approach(chatgpt_model, claude_model, deepseek_model, gemini_model, mistral_model):
    ...
    wb = Workbook()
    ws = wb.active
    workbook_filename = "Batch 4/Subset_Models_Results.xlsx"
    # Add a header row
    ...
    # CSV
    csv_filename = "Batch 4/Subset_Models_Results.csv"
    ...
    with open('datasets/Use Cases/3_use_cases.json', 'r') as file:
        ...
        for main_companies_index, company in enumerate(json_data['companies'][start_index:end_index], start=start_index):
            startup_name = company['company_name']
            print(f"JSON Index: {main_companies_index}\nStartup Name:{startup_name}")
